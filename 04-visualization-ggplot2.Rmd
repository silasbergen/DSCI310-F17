...
source("setup.R")
surveys_complete <- read.csv(file = "data/surveys.csv")
```
...
surveys_complete <- read.csv('data/surveys.csv')
```
...
## Is this a good way to show this type of data?
ggplot(data = surveys_complete, aes(x = species_id, y = weight,color=species_id)) + geom_point()
```
...
```{r sex-bargraph}
ggplot(data = surveys_complete, aes(x = sex)) +
    geom_bar()
```
