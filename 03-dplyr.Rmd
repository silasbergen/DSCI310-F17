...
Note that when piping into a function with no additional arguments, you can call the
...
Using `head()` function to look at resulting `data.frame`.
...
don't have any record in that plot. 

Relationships amongst genus can viewed using the `pairs()` function.  The first column contians labeling information regarding the `plot_id` and thus will be excluded in this plot.
...
> ### Challenge
> Run the `pairs()` function on the `surveys_gw` `data.frame`. 
>
>  `pairs(surveys_gw)`
>
